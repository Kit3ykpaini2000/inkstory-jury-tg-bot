...
# Добавляем корень проекта в sys.path — чтобы импорты работали при запуске из любой папки
...
async def _run_parser() -> int | None:
    """
    Запускает main.py как subprocess.
    Возвращает количество новых постов или None при ошибке.
    Работает на Windows и Raspberry Pi.
    """
    ...
    async with _parser_lock:
        try:
            ...
            if proc.returncode != 0:
                ...
                log.error(f"[parser] Ошибка (код {proc.returncode}): {err}")
                return None

            # Читаем assigned dict из последней строки stdout
            assigned = {}
            try:
                out = stdout.decode("utf-8", errors="replace").strip()
                last_line = out.splitlines()[-1] if out else ""
                assigned = json.loads(last_line)
            except Exception:
                pass

            queue_after = _get_queue_count()
            log.info(f"[parser] Готово. В очереди: {queue_after}")
            return assigned
